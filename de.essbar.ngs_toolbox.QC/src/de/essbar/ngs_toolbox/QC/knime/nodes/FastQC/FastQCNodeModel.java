...
/**
    @brief FastQC Node Model.
*/
public class FastQCNodeModel extends GenericKnimeNodeModel implements WizardNode<JSViewRepresentation, JSViewValue> { 
    ...
    protected FastQCNodeModel(INodeConfiguration nodeConfig,
            IPluginConfiguration pluginConfig) {
        //super(nodeConfig, pluginConfig, new String[][]{{"fq" ,"fastq" ,"sam" ,"bam" }, {"txt" ,"tsv" ,"csv" }, {"txt" ,"tsv" ,"csv" },}, new String[][]{{"inactive","*"},});
       // super(nodeConfig, pluginConfig, new String[][]{{"fq" ,"fastq" ,"sam" ,"bam" }, {"txt" ,"tsv" ,"csv"}, {"txt" ,"tsv" ,"csv"},}, new String[][]{});
      
        super(nodeConfig, pluginConfig, new String[][]{{"fq" ,"fastq" ,"sam" ,"bam" }, {"txt" ,"tsv" ,"csv"}, {"txt" ,"tsv" ,"csv"},}, new String[][]{{"*.html" ,"inactive" },});
    ...
    protected PortObject[] execute(PortObject[] inObjects,
        ...
        for (Parameter<?> param : super.getNodeConfiguration().getParameters()) {
        	...
        	if (param.getKey().matches("output")) {
    	...
    	/*
    	File dir = new File(html_file.getParentFile().toString());
    	
        List<URIContent> uris = new ArrayList<URIContent>();
        Collection<File> files_list_ext = FileUtils.listFiles(dir, new String[]{"html"}, true); 
        if (files_list_ext.isEmpty()) {
             throw new Exception("Could not find required file for index with extension html");
        }

        uris.add(new URIContent(dir.toURI(),"*"));
        
     	PrefixURIPortObject uri_prefix_object = null;
     	uri_prefix_object = new PrefixURIPortObject(uris,  dir.toString());
  		*/
