...
def create_config(config_file):
    mlstdb_dir = os.getcwd()
    print("create config file for MLST data in " + mlstdb_dir)
    #os.chdir(mlstdb_dir)
    allele_files = glob.glob('*.tfa')
    profile_file = glob.glob('*.txt')
    print(profile_file)
    f = open(config_file, "w")
    f.write("[loci]\n")
    for file in allele_files:
       print(file)
       file_path = os.path.join(mlstdb_dir, file)
    ...
    f.write("profile")
    f.write("\t")
    f.write(os.path.join(mlstdb_dir, profile_file[0]))
    f.write("\n")
    f.close()
...
def main():
	...
	scheme_dir = os.path.splitext(profile_filename)[0]
        os.mkdir(scheme_dir)
        os.chdir(scheme_dir)
